...
$(document).on("change", "#Group_setting_id", function () {
    ...
    $.ajax({
        url: "/Receivable/Group_setting/HasPassword?Id=" + GroupSettingId,
        method: "POST",
        success: function (HasPassword) {
            if (HasPassword) {
                ModelMsg("<div class='col-sm-12'><label>Enter Password</label><input type='password' class='GroupSettingPas form-control'  /></div>", "Enter Password"
                    ...
                    function () {
                        $.ajax({
                            url: "/Receivable/Group_setting/CheckPass?Id=" + GroupSettingId + "&Password=" + $(document).find(".GroupSettingPas").val(),
                            method: "POST",
...
function GetGroupSetting(GroupSettingId) {
    $.ajax({
        url: "/Receivable/Group_setting/GetGroupSeeting?Id=" + GroupSettingId,
        method: "POST",
        success: function (data) {
            $("#Currency_id").val(data.GS.Currency_id);
            ...
            $("#Account_Receivable_id").val(data.PGA.Account_Receivable_id)
            $("#Sales_id").val(data.PGA.Sales_id)
            $("#Discount_id").val(data.PGA.Discount_id)
            $("#Sales_variance_id").val(data.PGA.Sales_variance_id)
            $("#Accrued_sales_id").val(data.PGA.Accrued_sales_id)
