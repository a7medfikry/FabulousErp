...
namespace Payable.Controllers
{
    public class VoidController : Controller
    {
        ...
        public ActionResult Index(string Type,int Id)
        {
            ...
            {
                Payable_transaction PT= db.Payable_transactions.FirstOrDefault(x => x.Id == Id);
                ...
                ViewBag.RedirectLink = "/Payable/Payable_transaction?section=void";
            }
            else if (Type == TrxPay.Pay.ToString())
            {
                Payable_payment PP = db.Payable_payments.FirstOrDefault(x => x.Id == Id);
                ...
                ViewBag.RedirectLink = "/Payable/Payable_payment?section=void";

            }
            else if (Type == "Assign")
            {
                Assign_payable_doc AP = db.Assign_payable_docs.FirstOrDefault(x => x.Id == Id);
                ...
                ViewBag.RedirectLink = "/Payable/Assign_payable_doc?section=void";
            }
            return View();
        }
        public JsonResult CreateVoid(string Type, int Id,DateTime Transaction_date, DateTime Posting_date,string Reference)
        {
            ...
            if (Type == TrxPay.Trx.ToString())
            {
                Payable_transaction PTX = db.Payable_transactions.Find(Id);
                PTX.Is_void = true;
                List<int> OtherPayment =db.Related_pay_trans.Where(x=>x.Transaction_id== PTX.Trans_doc_type_id).Select(x => x.Payment_id).ToList();

                if (db.Assign_payable_docs.Any(x => x.Trans_doc_type_id_to == PTX.Trans_doc_type_id)
                    ||db.Assign_payable_docs.Any(x => OtherPayment.Any(z=>z==x.Trans_doc_type_id)))
                {
                    return Json("This Transaction Has Assign You Can't Void It");
                }
                db.Payable_void.Add(new Payable_void
                {
                ...
                if (db.Payable_payments.Any(x => x.Journal_number == PTX.Journal_number))
                {
                    int PId = db.Payable_payments.FirstOrDefault(x => x.Journal_number == PTX.Journal_number).Id;
            ...
            {
                Assign_payable_doc A = db.Assign_payable_docs.Find(Id);

                db.Assign_void.Add(new Payable_Assign_void
                {
                ...
                db.Assign_payable_docs.Remove(A);
        ...
        {
            Payable_payment PP = db.Payable_payments.Include(x => x.CheckBook_transaction).Include(x=>x.Trans_doc_type).FirstOrDefault(x => x.Id == Id);

            
            if (db.Assign_payable_docs.Any(x => x.Trans_doc_type_id == PP.Trans_doc_type_id))
            {
                return -1;
            }
