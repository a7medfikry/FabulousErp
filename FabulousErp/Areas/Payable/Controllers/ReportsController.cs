...
namespace Payable.Controllers
{
    public class ReportsController : Controller
    {
        ...
        {
            ViewBag.VendoreId = new SelectList(db.Payable_creditor_setting, "Id", "Vendor_id");
            ViewBag.ClassId = new SelectList(db.Payable_Group_settings, "Id", "Group_id");
        ...
        public ActionResult VendoreBalanceRes(int? VendoreId = null, int? GroupId = null)
        {
            ...
            List<Assign_payable_doc> APD = new List<Assign_payable_doc>();
            Date_option AgingOption = db.Aging_date_option.FirstOrDefault().Date_option;
            List<Payable_aging_period> AP = db.Aging_periods.OrderBy(x => x.From).ToList();
            List<TrxPayNumber> TPA = new List<TrxPayNumber>();

            if (VendoreId != null)
            {
                TPA.AddRange(db.Payable_transactions.Where(x => x.Is_void == false&&x.Vendor_id== VendoreId).Select(i => new TrxPayNumber
                {
                ...
                TPA.AddRange(db.Payable_payments.Where(x => x.Is_void == false&&x.Vendor_id==VendoreId).Select(i => new TrxPayNumber
                {
            ...
            {
                TPA.AddRange(db.Payable_transactions.Where(x => x.Is_void == false).Select(i => new TrxPayNumber
                {
                    Amount_of_transaction = (i.Purchase - i.Taken_discount + i.Tax) * i.Transaction_rate,
                ...
                TPA.AddRange(db.Payable_payments.Where(x => x.Is_void == false).Select(i => new TrxPayNumber
                {
                    Amount_of_payment = (i.Orginal_amount) * i.Transaction_rate,
                    Number_of_payment = 1,
                    Vendor_id = i.Vendor_id,
            ...
            if (VendoreId != null)
            {
                PP = db.Payable_payments.Where(x => x.Vendor_id == VendoreId && x.Is_void == false && x.Transaction_id == null).Include(x => x.Vendor).ToList();
                PT = db.Payable_transactions.Where(x => x.Vendor_id == VendoreId && x.Is_void == false).Include(x => x.Vendor).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
                APD = db.Assign_payable_docs.Where(x => x.Vendor_id == VendoreId && x.Is_void == false).ToList();
            }
            else if (GroupId != null)
            {
                List<Payable_Group_setting> GS = db.Payable_Group_settings.Where(x => x.Id == GroupId).ToList();
                PP = db.Payable_payments.Where(x => x.Transaction_id == null && x.Is_void == false).Include(x => x.Vendor).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id) && x.Is_void == false).ToList();
                PT = db.Payable_transactions.Include(z => z.Vendor).Where(x => x.Vendor_id != null && x.Is_void == false).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id)).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
                APD = db.Assign_payable_docs.Include(z => z.Vendor).Where(x => x.Vendor.Group_setting_id == GroupId && x.Is_void == false).ToList();
            }
            else
            {
            ...
            {
                PT = Business.GetUnpaidTransaction(VendoreId.Value);
                PP = Business.GetUnAssignPayment(VendoreId.Value, 0, false);
            }
            else
            {
                PT = Business.GetUnpaidTransaction();
                PP = Business.GetUnAssignPayment(IncludeTrans: false);
            ...
            List<Payable_aging_period> AgingRes = Res0.SelectMany(x=>x.AgingAmount).ToList();
            List<Payable_aging_period> AgingORes = db.Aging_periods.OrderBy(x => x.From).ToList();
            foreach (Payable_aging_period i in AgingRes)
            {
        ...
        public ActionResult VendoreCurrentActivtiy(bool LastYear = false)
        {
            ...
            ViewBag.ClassId = new SelectList(db.Payable_Group_settings, "Id", "Group_id");
            ViewBag.Aging = new SelectList(db.Aging_periods, "Id", "Name");
            ViewBag.GetLastYear = LastYear;
            if (LastYear)
            {
                ViewBag.Title = "Vendore Last Activtiy";
            }
            else
            {
                ViewBag.Title = "Vendore Current Activtiy";
            ...
            , DateTime Time)
        {
            ...
            List<Payable_transaction> PT = new List<Payable_transaction>();
            Date_option AgingOption = db.Aging_date_option.FirstOrDefault().Date_option;
            //List<Payable_aging_period> AP = new List<Payable_aging_period>();
            ...
            if (VendoreId != null)
            {
                PP = db.Payable_payments.Include(x => x.Trans_doc_type).Where(x => x.Vendor_id == VendoreId && x.Is_void == false).ToList();
                PT = db.Payable_transactions.Where(x => x.Vendor_id == VendoreId && x.Is_void == false).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
            }
            else if (GroupId != null)
            {
                List<Payable_Group_setting> GS = db.Payable_Group_settings.Where(x => x.Id == GroupId).ToList();
                PP = db.Payable_payments.Include(x => x.Trans_doc_type).Include(x => x.Vendor).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id) && x.Is_void == false).ToList();
                PT = db.Payable_transactions.Include(z => z.Vendor).Where(x => x.Vendor_id != null && x.Is_void == false).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id)).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
            }
            if (VendoreIdFrom != null && VendoreIdTo != null)
            {
                PP = db.Payable_payments.Where(x => x.Vendor_id >= VendoreIdFrom && x.Vendor_id <= VendoreIdTo && x.Is_void == false).ToList();
                PT = db.Payable_transactions.Where(x => x.Vendor_id >= VendoreIdFrom && x.Vendor_id <= VendoreIdTo && x.Is_void == false).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
            }
            else if (ClassIdFrom != null && ClassIdTo != null)
            {
                List<Payable_Group_setting> GS = db.Payable_Group_settings.Where(x => x.Id <= ClassIdFrom && x.Id >= ClassIdTo).ToList();
                PP = db.Payable_payments.Include(x => x.Trans_doc_type).Include(x => x.Vendor).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id) && x.Is_void == false).ToList();
                PT = db.Payable_transactions.Include(z => z.Vendor).Where(x => x.Vendor_id != null && x.Is_void == false).ToList().Where(x => GS.Any(z => z.Id == x.Vendor.Group_setting_id)).ToList().Where(x => !PP.Any(z => z.Transaction_id == x.Id)).ToList();
            ...
            {
                if (db.Aging_date_option.FirstOrDefault().Date_option == Date_option.From_document_date)
                {
            ...
            foreach (Payable_transaction i in PT)
            {
                ...
                try
                {
                    ...
                    Res.Add(new VendoreCurrentActivtiy
                    {
                        ...
                        UnassignedAmount = i.Purchase - i.Taken_discount + i.Tax - db.Assign_payable_docs.Where(x => x.Trans_doc_type_id_to == i.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign=0}).Sum(x => x.Applay_assign)
                    });
                }
                catch
                { }
            ...
            foreach (Payable_payment i in PP)
            {
                try
                {
                    double DueDateNumbers = 1;
                    ...
                    try
                    {
                        ...
                        Res.Add(new VendoreCurrentActivtiy
                        {
                            DocType = db.Payable_transactions_types.Where(x => x.Id == i.Trans_doc_type_id).FirstOrDefault().Doc_type,
                            Value = i.Orginal_amount,
                            UnassignedAmount =i.Orginal_amount- db.Assign_payable_docs.Where(x => x.Trans_doc_type_id == i.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign=0}).Sum(x => x.Applay_assign)
                        });
                    }
                    catch
                    {
        ...
        public ActionResult AgingRpt()
        {
            ...
            ViewBag.Aging = new SelectList(db.Aging_periods.OrderBy(x => x.From).ToList(), "Id", "Name");
            ViewBag.Creditor = new SelectList(db.Payable_creditor_setting, "Id", "Vendor_name");
            ViewBag.Class = new SelectList(db.Payable_Group_settings, "Id", "Group_id");
        ...
        private List<AgingRpt> CalcAgingRpt(string DetailsSumary, DateTime? From, DateTime? To, List<IncludeEnum> Include, int? VendoreFrom, int? VendoreTo, int? ClassFrom, int? ClassTo, string MultiDev, string OCurrencyId, ref decimal ORate)
        {
            ...
            List<Payable_transaction> PT = new List<Payable_transaction>();
            List<Payable_payment> PP = new List<Payable_payment>();
            List<Assign_payable_doc> AP = new List<Assign_payable_doc>();
            List<Payable_aging_period> AgingPeriod = db.Aging_periods.OrderBy(x => x.From).ToList();
            if (db.Aging_date_option.FirstOrDefault() == null)
            {
                TempData["Error"] = $"{FabulousErp.Business.Translate("There are no Aging Date Option")}";
                return new List<AgingRpt>();
            }
            Date_option AO = db.Aging_date_option.FirstOrDefault().Date_option;
            ViewBag.AgingNames = AgingPeriod;
            ViewBag.DetailsSumary = DetailsSumary;
            string companyID = (string)FabulousErp.Business.GetCompanyId();
            PT = db.Payable_transactions.Where(x => x.Is_void == false).Include(x => x.Vendor).Include(x => x.Trans_doc_type).Include(x => x.Currency).ToList();
            PP = db.Payable_payments.Where(x => x.Is_void == false).Include(x => x.Vendor).Include(x => x.Trans_doc_type).Include(x => x.Currency).ToList();
            List<Payable_aging_period> MyAging = new List<Payable_aging_period>();

            AP = db.Assign_payable_docs.Where(x => x.Is_void == false).Include(x => x.Trans_doc_type).Include(x => x.Trans_doc_type_to).Include(x => x.Currency).Include(x => x.Vendor).ToList();
            ...
            if (Include != null)
            {
                ...
                else
                {
                    ...
                    PP.ForEach(x => x.Orginal_amount -= db.Assign_payable_docs.Where(z => z.Trans_doc_type_id == x.Trans_doc_type_id && z.Is_void == false)
                                                         .ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign = 0 }).Sum(z => z.Applay_assign));
                }
            }
            else
            {
            ...
            Res.AddRange(PT.Select(x => new Controllers.AgingRpt
            {
                ...
                && x.Doc_type != Doc_type.Return) ? (x.Purchase - x.Taken_discount + x.Tax
                 - db.Assign_payable_docs.Where(z => z.Trans_doc_type_to.Payable_transaction.Any(y => y.Trans_doc_type_id == x.Trans_doc_type_id)).ToList().DefaultIfEmpty(new Assign_payable_doc {Taken_discount = 0 }).Sum(z => z.Taken_discount))
                   :
                   (x.Purchase - x.Taken_discount + x.Tax)
                   + db.Assign_payable_docs.Where(z => z.Trans_doc_type_id == x.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign = 0 }).Sum(z => z.Applay_assign),


                Amount = (x.Doc_type != Doc_type.Credit_Memo && x.Doc_type != Doc_type.Return) ?
                    (((x.Purchase - x.Taken_discount + x.Tax) * x.Transaction_rate)
                   - db.Assign_payable_docs.Where(z => z.Trans_doc_type_to.Payable_transaction.Any(y => y.Trans_doc_type_id == x.Trans_doc_type_id)).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign = 0, Taken_discount = 0, Transaction_rate = 1 }).Sum(z => z.Taken_discount * z.Transaction_rate))
                   :
                   (((x.Purchase - x.Taken_discount + x.Tax) * x.Transaction_rate)
                   + db.Assign_payable_docs.Where(z => z.Trans_doc_type_id == x.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign = 0, Taken_discount=0, Transaction_rate=1 }).Sum(z => z.Taken_discount + z.Applay_assign * z.Transaction_rate)),
              ...
              (MBus.PostiveSubtract(x.Posting_date, DateTime.Now)),
                AgingAmount = new List<Payable_aging_period> { },
            ...
            Res.AddRange(AP.Select(x => new Controllers.AgingRpt
            {
                ...
                Transaction_id = x.Trans_doc_type_to.Payable_transaction.FirstOrDefault().Id,
            ...
            if (Include != null && Include.Any(x => x == IncludeEnum.Fully))
            {
                Res.AddRange(PP.Select(x => new Controllers.AgingRpt
                {
                    ...
                    TransactionNo = x.Trans_doc_type.Trx_num.ToString(),
                    Currency = x.Currency.ISOCode,
                    OrginalAmount = -((x.Orginal_amount) - db.Assign_payable_docs.Where(z => z.Trans_doc_type_id == x.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign = 0 }).Sum(z => z.Applay_assign)),
                    Rate = x.Transaction_rate,
                    Type = x.Trans_doc_type.Doc_type.ToString(),
                    Amount = -((x.Orginal_amount - db.Assign_payable_docs.Where(z => z.Trans_doc_type_id == x.Trans_doc_type_id).ToList().DefaultIfEmpty(new Assign_payable_doc { Applay_assign=0 }).Sum(z => z.Applay_assign)) * x.Transaction_rate),
                    DueDays = (AO == Date_option.From_due_date) ?
              (x.Due_date.HasValue ? MBus.PostiveSubtract(x.Due_date.Value, DateTime.Now) : MBus.PostiveSubtract(x.Transaction_date, DateTime.Now)) :
              (MBus.PostiveSubtract(x.Posting_date, DateTime.Now)),
                    AgingAmount = new List<Payable_aging_period> { },
            ...
            Res.Where(x => x.DueDays != -1 && x.Amount != 0)
                .ToList()
                .ForEach(x => x.AgingAmount.Add(new Payable_aging_period
                {
                    Amount = x.Amount,
                    Id = AgingPeriod.Where(z => z.From <= x.DueDays && z.To >= x.DueDays).ToList().DefaultIfEmpty(new Payable_aging_period { Id = AgingPeriod.ToList().LastOrDefault().Id }).FirstOrDefault().Id,
                    Name = AgingPeriod.Where(z => z.From <= x.DueDays && z.To >= x.DueDays).ToList().DefaultIfEmpty(new Payable_aging_period { Name = AgingPeriod.ToList().LastOrDefault().Name }).FirstOrDefault().Name,
                    From = AgingPeriod.Where(z => z.From <= x.DueDays && z.To >= x.DueDays).ToList().DefaultIfEmpty(new Payable_aging_period { From = AgingPeriod.ToList().LastOrDefault().From }).FirstOrDefault().From,
                    To = AgingPeriod.Where(z => z.From <= x.DueDays && z.To >= x.DueDays).ToList().DefaultIfEmpty(new Payable_aging_period { To = AgingPeriod.ToList().LastOrDefault().To }).FirstOrDefault().To,
                }));
            ...
            if (Include != null)
            {
                ...
                {
                    Res.AddRange(db.Payable_creditor_setting.ToList().Where(x => !Res.Any(z => z.VendoreId == x.Id))
        ...
        public ActionResult ClasicAging()
        {
            ...
            ViewBag.Vendore = new SelectList(db.Payable_creditor_setting, "Id", "Vendor_name");
        ...
        public ActionResult ClasicAgingRes(int VendoreId,int Year)
        {
           ...
           List<Payable_transaction> PT = db.Payable_transactions.Where(x=>x.Is_void==false)
                .Include(x=>x.Currency).Include(x=>x.Trans_doc_type).ToList();
            List<Payable_payment> PP = db.Payable_payments.Where(x=>x.Is_void==false)
                 .Include(x => x.Currency).Include(x => x.Trans_doc_type).ToList();
            ...
            AC.AddRange(PT.Select(x => new AgingClasic
            {
                ...
                Desc = x.Desc,
                Debit = (x.Doc_type == Doc_type.Credit_Memo || x.Doc_type == Doc_type.Return 
                      || x.Doc_type == Doc_type.Payment)
                      ?(x.Purchase-x.Taken_discount+x.Tax):0,
                Credit=(x.Doc_type==Doc_type.Invoice||x.Doc_type==Doc_type.Debit_Memo)?
                (x.Purchase-x.Taken_discount+x.Tax):0,
                Currency=x.Currency.ISOCode,
                Rate=x.Transaction_rate,
                Doc_type = x.Trans_doc_type.Doc_type,
            ...
            AC.AddRange(PP.Select(x => new AgingClasic
            {
                ...
                Posting_date = x.Posting_date,
                Desc = x.Reference,
                Debit = x.Orginal_amount,
                Credit=0,
                Currency=x.Currency.ISOCode,
                Rate=x.Transaction_rate,
                Doc_type=x.Trans_doc_type.Doc_type,
                Doc_num=x.Trans_doc_type.Counter.ToString()
            }));
            ...
            AC.AddRange(db.Assign_payable_docs.Where(x => x.Taken_discount != 0).ToList().Select(x => new AgingClasic
            {
                JVNo = x.JournalEntry,
                Posting_date = x.Applay_date,
                Desc =BusController.Translate("Assign Payalbe document"),
                Debit = x.Taken_discount,
                Credit = 0,
                Currency = x.Currency.ISOCode,
                Rate =x.Transaction_rate,
            ...
            AC.Add(new AgingClasic
            {
                ...
                Debit=0,
                Currency="",
                Desc=BusController.Translate("Begingn Balance"),
                IsFirst=true
            });
            return View(AC);
        }
    ...
    public class VendoreBalance
    {
       ...
       public List<Payable_aging_period> Aging { get; set; }
    }
    ...
    public class AgingRpt
    {
        ...
        public List<Payable_aging_period> AgingAmount { get; set; }
